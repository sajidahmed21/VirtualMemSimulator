...
// Used by get_frame_index() to indicate no frame corresponds to a given vaddr
...
extern int get_frame_index(addr_t addr);
...
struct frame {
	...
	// Pointer to the frame that was accessed right after this frame
	struct frame *next;
	
	// Pointer to the frame that was accessed right before this frame
	struct frame *prev;
