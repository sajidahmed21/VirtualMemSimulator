...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		pgtbl_entry_t *victim_pgtbl_entry = coremap[frame].pte;
		off_t swap_offset = victim_pgtbl_entry->swap_off;
		
		// Write victim page to swap file only if it is dirty
		if (victim_pgtbl_entry->frame & PG_DIRTY) {
			
			int swapout_result = swap_pageout((unsigned) frame, swap_offset);
			
			if (swapout_result == INVALID_SWAP) {
				fprintf(stderr, "Error: Swap page out failed\n");
				exit(EXIT_FAILURE);
			}
			
			// Set swap offset in pte
			victim_pgtbl_entry->swap_off = swapout_result;
			
			// Indicate page has been evicted to swap
			victim_pgtbl_entry->frame |= PG_ONSWAP;
		...
		// Update pte to indicate virtual page no longer dirty
		victim_pgtbl_entry->frame &= ~PG_DIRTY;
		
		// Update pte to indicate virtual page no longer in physical memory
		victim_pgtbl_entry->frame &= ~PG_VALID;
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	unsigned pgdir_index = PGDIR_INDEX(vaddr); // get index into page directory

	// Use top-level page directory to get pointer to 2nd-level page table
	pgdir_entry_t *pgdir_entry = pgdir + pgdir_index;
	
	if (!(pgdir_entry->pde & PG_VALID)) { // pgdir entry not valid
		*pgdir_entry = init_second_level();
	}

	// Use vaddr to get index into 2nd-level page table and access the 'pte'
	unsigned pgtbl_index = PGTBL_INDEX(vaddr);
	
	p = ((pgtbl_entry_t *) (pgdir_entry->pde & ~1)) + pgtbl_index;
	
	if (!(p->frame & PG_VALID)) { // pte not valid (i.e. miss)
		
		miss_count++;
	    int frame = allocate_frame(p);
		p->frame = (frame << PAGE_SHIFT) | (p->frame & ~PAGE_MASK) | PG_VALID;
		
		// Read page data if available from swap file
		if (p->frame & PG_ONSWAP) {
			swap_pagein(frame, p->swap_off);
		}
		
		// Intialize frame if not available in swap file
		else {
	...
	else { // pte valid (i.e. hit)
		hit_count++;
	}
	
	// Mark page as dirty if it will be written to
	if (type == 'S' || type == 'M') {
		p->frame |= PG_DIRTY;
	}
...
/* Returns the physical frame number for the given vaddr using the page table.
 *
 * Returns PAGE_NOT_IN_MEMORY if page being referred by the given vaddr
 * is not in memory.
 */
int get_frame_index(addr_t vaddr) {
	unsigned pgdir_index = PGDIR_INDEX(vaddr);
	pgdir_entry_t *pgdir_entry = pgdir + pgdir_index;
	
	if (!(pgdir_entry->pde & PG_VALID)) {
		return PAGE_NOT_IN_MEMORY;
	}
	
	// Access page table entry
	unsigned pgtbl_idx = PGTBL_INDEX(vaddr);
	pgtbl_entry_t *p = ((pgtbl_entry_t *) (pgdir_entry->pde & ~1)) + pgtbl_idx;
	
	if (!(p->frame & PG_VALID)) {
		return PAGE_NOT_IN_MEMORY;
	}
	
	return p->frame >> PAGE_SHIFT;
}
